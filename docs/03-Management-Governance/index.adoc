...
The LzaDeployment OU houses the dedicated account for container-based LZA deployments. The LZA Universal Config applies specialized SCPs to this OU that protect LZA-managed resources from unauthorized modification and deletion while permitting operators to perform deployment operations.

NOTE: This OU is applicable only to container-based LZA deployments in partitions that require it, such as the AWS European Sovereign Cloud. Standard AWS CodePipeline-based deployments do not use this OU.
...
===== LzaDeployment Account

The LzaDeployment account hosts the deployment infrastructure for the container deployment method. LZA depends on services like AWS CodeBuild and AWS CodePipeline, which may not be available in all AWS Regions. In these Regions, LZA is deployed using Amazon Elastic Container Service (Amazon ECS) with AWS Fargate. This account contains the orchestration infrastructure (Amazon ECS cluster, container image, AWS Systems Manager Automation) that deploys and manages LZA resources in the Management account and across the organization. Because this account has cross-account access to the Management account, it requires the same level of access restriction.

The LZA Universal Config applies protective SCPs that prevent operators from modifying or deleting LZA-managed resources while permitting the actions required for deployment operations: uploading configuration, triggering automation, and monitoring execution.

NOTE: This account is applicable only to container-based LZA deployments in partitions that require it, such as the AWS European Sovereign Cloud. Standard AWS CodePipeline-based deployments do not require a separate deployment account.
...
Review Amazon S3 backup [documentation](https://docs.aws.amazon.com/aws-backup/latest/devguide/s3-backups.html) for bucket prerequisites (e.g. Versioning enabled).
