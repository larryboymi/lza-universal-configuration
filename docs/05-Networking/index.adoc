...
The central endpoints VPC includes https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/vpc-interface-endpoints.html[Amazon Bedrock AgentCore PrivateLink endpoints] for private access to AI agent services:

* `bedrock-agentcore` — Data plane (Runtime, Memory, Code Interpreter, Browser, Identity)
* `bedrock-agentcore-control` — Control plane (Runtime/Memory management)
* `bedrock-agentcore.gateway` — Gateway invocation

Restrictive endpoint policies (`AgentCore-DataPlane`, `AgentCore-Gateway`) are provided for Prod environments. These scope allowed actions to data-plane operations only. For OAuth-authenticated Gateway traffic, the `Principal` must be `"*"` as https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/vpc-interface-endpoints.html[documented by AWS] — VPC endpoint policies cannot restrict based on OAuth identity.

[IMPORTANT]
====
Before applying the restrictive endpoint policies, customers must:

1. *Understand the organization boundary enforcement*:
** Both `AgentCore-DataPlane` and `AgentCore-Gateway` use https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies_condition-keys.html#condition-keys-resourceorgid[`aws:ResourceOrgID`] to ensure that only AgentCore resources belonging to your organization can be accessed through the endpoint. This approach works for both IAM (SigV4) and OAuth callers because it restricts based on the *resource being accessed* rather than the caller's identity. The endpoint cannot be used to reach AgentCore resources in other organizations.
** For customers who use only SigV4 authentication (no OAuth), the data plane policy can be further tightened by replacing `aws:ResourceOrgID` with https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies_condition-keys.html#condition-keys-principalorgid[`aws:PrincipalOrgID`] to restrict by caller identity. However, this will block OAuth callers — use only if your Runtimes are exclusively SigV4-authenticated.
2. *Regional availability*: AgentCore PrivateLink endpoints are not available in all regions. Verify availability in your deployment region before enabling. See https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/agentcore-regions.html[AgentCore supported regions].
3. *Apply policies per-endpoint*: To use the restrictive policies instead of the default, set `applyPolicy: true` and `policy: AgentCore-DataPlane` (or `AgentCore-Gateway`) on the relevant endpoint entry in `network-config.yaml`.
====
...
https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/resolver-dns-firewall.html[Route 53 Resolver DNS Firewall] filters outbound DNS queries from VPCs, blocking resolution of domains associated with known threats. This provides DNS-layer protection against data exfiltration, malware communication, and botnet command-and-control activity across all VPCs in the landing zone.

The LZA Universal Configuration deploys a DNS Firewall rule group using https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/resolver-dns-firewall-managed-domain-lists.html[AWS-managed domain lists]:

* *AWSManagedDomainsAggregateThreatList*: Consolidated list of domains associated with multiple threat categories
* *AWSManagedDomainsMalwareDomainList*: Domains hosting or distributing malware
* *AWSManagedDomainsBotnetCommandandControl*: Domains used for botnet command-and-control communication

The rule group is:

* Shared via https://docs.aws.amazon.com/ram/latest/userguide/what-is.html[AWS RAM] to Workloads/Dev, Workloads/Test, Workloads/Prod OUs and SharedServices, Network, and Perimeter accounts
* Associated with all LZA-managed VPCs: Endpoints, Inspection, Ingress, Egress, SharedServices, Dev, Test, and Prod
* Explicitly excluded from Sandbox (no LZA-managed VPC deployed there)
* Protected with `mutationProtection: ENABLED` to prevent unauthorized disassociation

NOTE: The Network account owns the DNS Firewall rule groups and accesses them directly without RAM sharing. The Network account should not be included in `shareTargets` as the owning account does not need to be a share recipient. Sandbox accounts have no LZA-managed VPC and therefore no DNS Firewall association. This is by design — Sandbox allows unrestricted DNS for rapid iteration.

*Cost Impact*::
Route 53 Resolver DNS Firewall charges https://aws.amazon.com/route53/pricing/[$0.60 per million DNS queries processed] for the first billion queries, and $0.40 per million thereafter. There is no charge for the rule group itself or for using AWS-managed domain lists. For a typical workload VPC generating 1–5 million DNS queries per month, the cost is under $3/month per VPC. This applies to all DNS queries originating from within the VPC, regardless of whether they match a rule.

=== DNS Query Logging

https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/resolver-query-logs.html[Route 53 Resolver Query Logging] captures DNS queries originating from all VPCs in the environment. This provides visibility into DNS activity for security monitoring, threat detection, and incident response.

The Universal Configuration enables query logging across all VPCs by:

* Creating a centralized query logging configuration shared via RAM to all accounts and OUs
* Explicitly associating each VPC with the query logging configuration
* Sending logs to CloudWatch Logs, which are then replicated to the centralized S3 bucket via dynamic partitioning

Query logs are stored in the centralized S3 bucket under the `route53-resolver-query-logs/` prefix.

NOTE: The query logging configuration follows the same sharing pattern as DNS Firewall rule groups. The Network account (delegated admin) owns the configuration and shares it with all workload OUs and the Perimeter and SharedServices accounts.

=== Transit Gateway Flow Logs

https://docs.aws.amazon.com/vpc/latest/tgw/tgw-flow-logs.html[Transit Gateway Flow Logs] capture IP traffic going to and from transit gateways. VPC flow logs only capture per-interface traffic; transit gateway flow logs show the cross-VPC and cross-account traffic traversing the hub.

The Universal Configuration enables transit gateway flow logs with:

* `maxAggregationInterval: 60` (the only supported value for transit gateways)
* Custom format with fields for TGW attachment identification, cross-account VPC tracking, packet loss metrics, and AWS service identification
* CloudWatch Logs destination, replicated to the centralized S3 bucket under the `tgw-flow-logs/` prefix
...
5. *Transit Gateway route table segmentation*: The default configuration implements two route tables (firewall and spoke) following the AWS Network Firewall centralized deployment model. Organizations with complex hybrid connectivity or strict environment isolation requirements may consider adding additional route tables for granular traffic control.
