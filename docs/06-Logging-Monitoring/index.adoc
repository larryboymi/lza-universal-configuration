...
|AWSAccelerator-*QueryLogsLogGroup* |route53-resolver-query-logs
|/aws/transitgateway/* |tgw-flow-logs
|/aws/lambda/* |lambda
...
Transit Gateway Flow Logs capture IP traffic going to and from transit gateways. VPC flow logs only capture per-interface traffic; transit gateway flow logs show cross-VPC and cross-account traffic traversing the hub.

The Universal Configuration enables transit gateway flow logs with a custom format:
...
Logs are sent to CloudWatch Logs and forwarded to the centralized S3 bucket with the prefix `tgw-flow-logs/`. `maxAggregationInterval` is 60 seconds, the only supported value for transit gateway flow logs.

=== Route 53 Resolver Query Logs

Route 53 Resolver Query Logging captures DNS queries originating from all VPCs in the environment. This provides visibility into DNS activity for security monitoring, threat detection, and compliance. The Universal Configuration enables query logging across all VPCs and centralizes the logs through CloudWatch Logs replication.

[yaml]
----
queryLogs:
  name: "{{ AcceleratorPrefix }}-{{ HomeRegion }}-query-logs"
  destinations:
    - cloud-watch-logs
  shareTargets:
    accounts:
...
Query logs are configured to send to CloudWatch Logs and are automatically forwarded to the centralized S3 bucket with the prefix `route53-resolver-query-logs/`. The query logging configuration is shared via RAM to all workload OUs, Infrastructure, SharedServices, and Perimeter accounts. Each VPC explicitly associates with the query logging configuration to ensure complete coverage.
