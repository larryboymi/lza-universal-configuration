...
These guardrails are consistently applied across the entire workload hierarchy—covering Development, Test, and Production organizational units—creating standardized security boundaries regardless of environment maturity. The controls focus specifically on protecting network architecture, enforcing data encryption, and securing AI agent infrastructure while allowing teams sufficient flexibility to deploy and manage application resources within these secure boundaries.
...
*STS (`GRSTSDPB`)*::
Prevents external principals from assuming IAM roles in member accounts via `AssumeRole`, `AssumeRoleWithSAML`, or `AssumeRoleWithWebIdentity`. This closes the most critical identity perimeter gap—an overly permissive role trust policy could otherwise allow any AWS principal to assume a role in your accounts.
